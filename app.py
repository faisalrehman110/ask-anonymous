...
def ensure_admin_user():
    conn = sqlite3.connect("data.db")
    c = conn.cursor()
    c.execute("SELECT id FROM users WHERE username=?", ("faisalansari",))
    if c.fetchone() is None:
        c.execute(
            "INSERT INTO users (id, username, email, password) VALUES (?, ?, ?, ?)",
            (str(uuid.uuid4()), "faisalansari", "faisalansari@admin.local", "Faisal@110"),
        )
        conn.commit()
    conn.close()
...
def login():
    if request.method == "POST":
        ...
        if user:
            ...
            session["is_admin"] = (user[1] == "faisalansari")
            if session["is_admin"]:
